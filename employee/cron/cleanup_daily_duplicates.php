<?php
/**
 * Cleanup Script: Remove duplicates from daily_payroll_reports
 * Aggregates duplicate records per employee/day/branch and keeps only one summary
 */
...
$log_file = __DIR__ . '/cleanup_daily_duplicates.log';
...
$log_message("=== Starting Daily Payroll Deduplication ===");

// First, check current state
$check_result = mysqli_query($db, "SELECT COUNT(*) as total, COUNT(DISTINCT CONCAT(employee_id, '-', report_date, '-', COALESCE(branch_id, 'NULL'))) as unique_records FROM daily_payroll_reports");
...
// Create backup table
$backup_table = "daily_payroll_reports_backup_" . date('Ymd_His');
mysqli_query($db, "CREATE TABLE IF NOT EXISTS `$backup_table` LIKE daily_payroll_reports");
mysqli_query($db, "INSERT INTO `$backup_table` SELECT * FROM daily_payroll_reports");
$log_message("Backup created: $backup_table");

// Get all duplicate groups
$duplicates_query = "SELECT employee_id, report_date, branch_id, COUNT(*) as count, MAX(id) as keep_id
                     FROM daily_payroll_reports 
                     GROUP BY employee_id, report_date, branch_id 
                     HAVING COUNT(*) > 1";
...
while ($dup = mysqli_fetch_assoc($dup_result)) {
    ...
    $agg_query = "SELECT 
        ...
        COALESCE(MAX(report_year), YEAR('$report_date')) as report_year,
        COALESCE(MAX(report_month), MONTH('$report_date')) as report_month,
        COALESCE(MAX(report_day), DAY('$report_date')) as report_day,
        COALESCE(MAX(week_number), 1) as week_number
    FROM daily_payroll_reports 
    WHERE employee_id = ? AND report_date = ? AND branch_id = ?";
    
    $agg_stmt = mysqli_prepare($db, $agg_query);
    mysqli_stmt_bind_param($agg_stmt, 'isi', $emp_id, $report_date, $branch);
    ...
    $report_year = intval($agg['report_year']);
    $report_month = intval($agg['report_month']);
    $report_day = intval($agg['report_day']);
    $week_number = intval($agg['week_number']);
    
    // Update using direct query
    $update_query = "UPDATE daily_payroll_reports SET
    ...
    $delete_query = "DELETE FROM daily_payroll_reports 
                     WHERE employee_id = $emp_id AND report_date = '$report_date' AND branch_id <=> $branch_val
                     AND id != $keep_id";
    ...
    $log_message("Cleaned: Emp $emp_id Date $report_date - Aggregated $count records, deleted $deleted duplicates");
}

// Verify results
$check_result2 = mysqli_query($db, "SELECT COUNT(*) as total FROM daily_payroll_reports");
