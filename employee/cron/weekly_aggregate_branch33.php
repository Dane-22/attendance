...
$log_file = __DIR__ . '/weekly_aggregate_branch33.log';
...
$log_message("=== Starting Weekly Aggregation (Branch 33) ===");

// Get current week info (Saturday = end of week for branch 33)
$today = date('Y-m-d');
$year = date('Y');
$month = date('n');
...
// Calculate week date range (Monday to Saturday for branch 33)
...
$log_message("Week $week_number (Branch 33): $start_date to $end_date");
...
while ($row = mysqli_fetch_assoc($result)) {
    ...
    if (mysqli_stmt_execute($insert_stmt)) {
        $total_records++;
        $log_message("Aggregated: Employee #$emp_id ($employee_name) - Branch: " . ($row['branch_name'] ?? 'Branch 33') . " - Days: " . $row['total_days'] . " - Net: " . $row['total_take_home']);
    } else {
...
$log_message("=== Weekly Aggregation (Branch 33) Complete ===\n");
