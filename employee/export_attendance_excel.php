...
if (empty($branchData)) {
    ...
    foreach ($branchData as $branchName => $employees) {
        ...
        $sheet->mergeCells('A' . $currentRow . ':F' . $currentRow);
        $branchHeaderStyle = $sheet->getStyle('A' . $currentRow . ':F' . $currentRow);
        ...
        // Column Headers
        $headers = ['Date', 'Name', 'Time in', 'Time out', 'Total hours', 'Status'];
        $col = 'A';
        foreach ($headers as $header) {
            $sheet->setCellValue($col . $currentRow, $header);
            $col++;
        }
        $colHeaderStyle = $sheet->getStyle('A' . $currentRow . ':F' . $currentRow);
        ...
        foreach ($employees as $emp) {
            $sheet->setCellValue('A' . $currentRow, date('M d, Y', strtotime($emp['date'])));
            $sheet->setCellValue('B' . $currentRow, strtoupper($emp['name']));
            $sheet->setCellValue('C' . $currentRow, $emp['time_in']);
            $sheet->setCellValue('D' . $currentRow, $emp['time_out']);
            $sheet->setCellValue('E' . $currentRow, $emp['hours']);
            $sheet->setCellValue('F' . $currentRow, $emp['status']);
            
            // Apply cell colors
            // Date column - light yellow
            $sheet->getStyle('A' . $currentRow)->getFill()->setFillType(Fill::FILL_SOLID)->getStartColor()->setRGB('FFF2CC');
            // Name column - light green
            $sheet->getStyle('B' . $currentRow)->getFill()->setFillType(Fill::FILL_SOLID)->getStartColor()->setRGB($nameBg);
            ...
            $sheet->getStyle('E' . $currentRow)->getFill()->setFillType(Fill::FILL_SOLID)->getStartColor()->setRGB($timeBg);
            // Status column - light salmon/orange
            $sheet->getStyle('F' . $currentRow)->getFill()->setFillType(Fill::FILL_SOLID)->getStartColor()->setRGB($statusBg);
